...
The RNN block consist of single computation layer with $tanh$ activation function. Compared to the Feedforward Neural Network, RNN does not take decisions based on only the current input. RNN finds the pattern dependencies based on current and previous inputs. RNN has $h_{t}$ function of the previous cell state $h_{t-1}$ and current input$ X_{t}$. LSTM is compexer and consists of several computational blocks that control information flow of information through the cell. The key building block behind LSTM is a structure known as $gates$. They allow LSTM to avoid the weight conflict when making decision which information from the past and current timestamp is important for correct mapping inputs to outputs. In other words, network can decide using gates when to keep or override the information in memory cell and how to access the current memory cell \cite{lstm_orig}.
...
\begin{figure}[htb]
	...
	\end{center}
\end{figure}
The LSTM architecture is illustrated\footnote{Source: Prof. Dr. Tim Landgraf, Lecture 13: Recurrent Neural Networks, WS 20/21: Machine Learning} on Fig. \ref{fig:lstm}.
