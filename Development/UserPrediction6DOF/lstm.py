...
class LSTM(object):
    ...
    1. Input gate I_input uses two layers:
    ...
    2. Forget gate Ct keeps or discards information from the long-term memory.
    ...
    3. Output gate
        Takes the current input Xt, the previous short-term memory Ht_1 (hidden state)
        and long-term memory computed in current step and ouputs the new hidden state Ht
